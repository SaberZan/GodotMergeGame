...
import { WaitAciton } from "./actions/WaitAction";
...
export class BehaviorTreeBuilder<T>{
    ...
    private setChildOnParent(child: Behavior<T>): BehaviorTreeBuilder<T> {
        let parent = this._parentNodeStack[0];
    ...
    private pushParentNode(composite: Behavior<T>) {
        ...
        this._parentNodeStack.splice(0, 0, composite);
        return this;
    }

    private endDecorator(): BehaviorTreeBuilder<T> {
        this._currentNode = this._parentNodeStack.shift();
        return this;
    }
    ...
    public endComposite(): BehaviorTreeBuilder<T> {
        Assert.isTrue(this._parentNodeStack[0] instanceof Composite, "尝试结束复合器，但顶部节点是装饰器");
        this._currentNode = this._parentNodeStack.shift();
        return this;
    }
